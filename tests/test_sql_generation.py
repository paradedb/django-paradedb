...
class TestBoosting:
    ...
    def test_boost_match_distance(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Match(Boost(Fuzzy("shose", 2), 2.0), operator="OR"))
        )
        assert (
            str(queryset.query)
    ...
    def test_boost_phrase(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Phrase(Boost("running shoes", 1.5)))
        )
        assert (
            str(queryset.query)
    ...
    def test_boost_regex(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Boost(Regex("key.*"), 2.0))
        )
        assert (
            str(queryset.query)
    ...
    def test_boost_parse(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Boost(Parse("shoes"), 3.0))
        )
        assert (
            str(queryset.query)
    ...
    def test_boost_term(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Boost(Term("shoes"), 2.0))
        )
        assert (
            str(queryset.query)
...
class TestConstantScoring:
    ...
    def test_const_plain_string(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Match(Const("shoes", 1.0), operator="AND"))
        )
        assert (
            str(queryset.query)
    ...
    def test_const_regex(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Const(Regex("key.*"), 1.0))
        )
        assert (
            str(queryset.query)
    ...
    def test_const_term(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Const(Term("shoes"), 2.0))
        )
        assert (
            str(queryset.query)
    ...
    def test_boost_and_const_phrase_sql_is_generated(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Phrase(Const(Boost("x", 2.0), 1.0)))
        )
        assert (
            str(queryset.query)
    ...
    def test_boost_multi_term_plain_strings(self) -> None:
        queryset = Product.objects.filter(
            description=ParadeDB(Match(Boost(("shoes", "boots"), 2.0), operator="AND"))
        )
        assert (
            str(queryset.query)
