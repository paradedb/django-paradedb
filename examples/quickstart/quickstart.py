...
from paradedb.search import Match, Phrase
...
def demo_scored_search() -> None:
    ...
    results = (
        MockItem.objects.filter(description__pdb=Match("running", operator="AND"))
...
def demo_phrase_search() -> None:
    ...
    results = (
        MockItem.objects.filter(description__pdb=Phrase("running shoes"))
        .annotate(score=Score())
        .order_by("-score")[:5]
    )
    for item in results:
